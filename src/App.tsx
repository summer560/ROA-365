...
import { alertMessages, dashboard, events, faqs, notices, noticeDetails, products, quickMenus, quickMenuDetails, roaServices, roaServiceDetails, safezoneFacilities, safezoneSteps } from './mockData';
...
function HomePage({ onOpenQuick, onBuyProduct, showDiscount }: { onOpenQuick: (name: string) => void; onBuyProduct: (name: string) => void; showDiscount: boolean }) {
  return <><Hero />{showDiscount && <DiscountTab />}<QuickMenu onOpen={onOpenQuick} /><StatusDashboard /><ProductSection onBuy={onBuyProduct} /><EventSection /><SafezoneSection /><ROAPartnershipSection /><HiringSection /><NoticeSection /></>;
}
